...
def test_build_table_fold_change():
    ...
    row = tbl[tbl["feature"] == "n_dpn"].iloc[0]
    assert abs(row["fc_2d"] - row["mudmut_mean_2d"] / row["wt_mean_2d"]) < 1e-10
    assert abs(row["fc_3d"] - row["mudmut_mean_3d"] / row["wt_mean_3d"]) < 1e-10
